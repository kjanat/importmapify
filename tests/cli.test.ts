...
describe('generateCommand', () => {
	...
	it('drops targets rejected by a --filter regex', async () => {
		const root = fixture({ '#dist/*': './dist/*.js' }, ['dist/auto.js', 'dist/internal-qo9O8jzH.js']);
		const result = await runCommand(generateCommand, ['--root', root, '--stdout', '--filter', '^(?!.*internal)']);
	...
	it('rejects a --filter value that is not a valid regex', async () => {
		const root = fixture({});
		const result = await runCommand(generateCommand, ['--root', root, '--stdout', '--filter', '(unclosed']);
		expect(result.exitCode).not.toBe(0);
	});

	it('indents output per --indent spaces', async () => {
		const root = fixture({ '#config': './src/config.ts' }, ['src/config.ts']);
		const result = await runCommand(generateCommand, ['--root', root, '--stdout', '--indent', '2']);
		expect(result.stdout.join('')).toBe('{\n  "imports": {\n    "#config": "./src/config.ts"\n  }\n}\n\n');
	});

	it('rejects an --indent value that is neither a number nor "tab"', async () => {
		const root = fixture({});
		const result = await runCommand(generateCommand, ['--root', root, '--stdout', '--indent', 'wide']);
		expect(result.exitCode).not.toBe(0);
	});
